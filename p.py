...
class KCScraper:
    ...
    def main(self):
        ...
        with open("proxy.txt", "w") as output:
    ...
    def getSettings(self) -> tuple:
        ...
        if presets == "?":
            ...
            if presets in yes:
                ...
                print(Colorate.Diagonal(
                    ...
                    Center.XCenter("\n[1] HTTP/S\t[2] SOCKS4\t[3] SOCKS5\t [4] ALL")))
        ...
        elif presets not in {"1", "2", "3", "4", "n"}:
            print(f"{self.prefix_error} Error in settings.json at presets")
            input()
            exit()

        config = {"1": "presets/http.txt", "2": "presets/socks4.txt", "3": "presets/socks5.txt", "4": "presets/all.txt", "n": "sites.txt",
                  "no": "sites.txt"}
    ...
    def terminal(self, string: str = ""):
        if os.name == "nt":
            ctypes.windll.kernel32.SetConsoleTitleW("Baidajah " + string)
